...
- **방치형 공장 게임** (`game/`) — 실제 1.0 레시피·전력·건설 비용 밸런스로 돌아가는
  브라우저 공장 건설 게임. `game/index.html` 을 열면 실행. 2D 캔버스에 채굴기·기계 노드를
  배치하고 출력→입력 포트를 드래그로 연결해 생산 체인을 설계한다 (버퍼·배압·전력 부족 감속
  시뮬레이션). 출하 노드에 연결된 아이템만 재고로 들어오고, 그 재고로 마일스톤 8개를 달성하는
  것이 목표. localStorage 자동 저장 + 오프라인 진행(최대 4시간).
